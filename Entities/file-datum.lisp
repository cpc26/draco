...
(declaim (function find-file-datum (file-spec) file-datum))
(declaim (function get-file-datum (string &rest args) file-datum))


;; internal method definitions
(defmethod get-file-data ((directory pathname))
	...
	(load (make-pathname	:defaults	directory
				:name		*inventory-name*
				:type		*inventory-type*)
			...
			(when (typep reporter 'KV-reporter)
				...
				(let ((keyword nil)
				     ...
				     (aver ((optional string) keyword value)
					   ((optional stream) ls-stream)
					   ((optional stream) ignored-stream)
					   ((optional integer) pid))
				     (unwind-protect
					(with-open-stream
					  (stream
					...
					(ignore-errors
					  (when pid
						(blocking-wait-with-pid pid)))
				     ))))))) ; unwind-protect...with-accessors
	(values))
